...
|---|---|---|
| **"≤5 s response per label or nobody uses it"** | Sarah Chen | Default extractor is Gemini 2.5 Flash. The original presearch baseline cited ~1.5 s; live measurement on Render is 7–9 s end-to-end (see `docs/ERROR_FIX_LOG.md` 2026-05-22). The LRU cache (`sha256(image_bytes)`) makes the second pass on the same image return in well under 100 ms — including the case where `/extract` populated it and the agent then clicks Verify. SSE in the batch flow shows progress, not a spinner; the upload-prefill flow shows an elapsed-seconds counter so a 7 s wait visibly progresses. The 5 s bar is not met on cache-miss real labels today; honest accounting in the README "Trade-offs" section. |
...
2. **Gemini 2.5 Flash primary, OpenAI GPT-4o fallback, both behind one `LabelExtractor` ABC.** Gemini was selected for the original ~1.5 s presearch baseline; live measurement on Render is 7–9 s end-to-end. Both vendors are on GSA MAS. The hand-rolled ABC (`app/extractors/base.py`, ~30 lines) shows the abstraction explicitly — adding a Bedrock-Claude extractor for a GovCloud-routed deploy is a new file, not a refactor.
