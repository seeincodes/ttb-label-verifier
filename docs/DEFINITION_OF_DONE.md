...
| 9 | Every verdict reason cites the specific 27 CFR section | ✅ | `FieldVerdict.model_validator` (`app/models.py`) refuses to construct a WARN/FAIL without `cfr_citation`. Verifier rules (`app/verifier/rules.py`) cite per-beverage-type. The consolidated CFR table lives in [`docs/DESIGN_NOTES.md §2`](DESIGN_NOTES.md). |
| 10 | Stakeholder-signals table | ✅ | [`docs/DESIGN_NOTES.md §3`](DESIGN_NOTES.md) — every Sarah / Dave / Jenny / Marcus signal from the discovery interviews mapped to a concrete code decision with `path/file.py:line` citations. (Lived in the original README §8; moved out of the README in the 2026-05-22 trim.) |
...
| 12 | Screenshots + GIF as deployment-failure backup | ✅ | 5 PNGs captured in `docs/screenshots/` (home, three samples, eval-empty), embedded in the README "Try it" section. Capture path documented in `docs/DEPLOY.md` §5 for future regeneration. |
...
- All items ✅ as of 2026-05-22: deployed at <https://ttb-label-verifier-pybx.onrender.com>; repo public at <https://github.com/seeincodes/ttb-label-verifier>; 5 screenshots inline in the README "Try it" section.
